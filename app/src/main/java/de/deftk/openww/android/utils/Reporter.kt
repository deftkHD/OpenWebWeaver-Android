...
object Reporter {
    ...
    fun reportUncaughtException(exception: Throwable, context: Context) {
        if (DebugUtil.areDevToolsEnabled(context)) {
            DebugUtil.reportException(ExceptionSource.CAUGHT, Thread.currentThread().stackTrace, exception, null)
        }
    ...
    fun reportException(@StringRes message: Int, exception: String, context: Context) {
        ...
        if (DebugUtil.areDevToolsEnabled(context)) {
            DebugUtil.reportException(ExceptionSource.LOGIC, Thread.currentThread().stackTrace, null, text)
        }
    ...
    fun reportFeatureNotAvailable(context: Context) {
        if (DebugUtil.areDevToolsEnabled(context)) {
            DebugUtil.reportException(ExceptionSource.FEATURE_NOT_AVAILABLE, Thread.currentThread().stackTrace, null, context.getString(R.string.feature_not_available))
        }
